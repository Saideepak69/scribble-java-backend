...
class Message {
    ...
    public Message(String type, String roomId, String username, Object payload) {
        this.type = type; this.roomId = roomId; this.username = username; this.payload = payload;
    }
}
...
class GameRoom {
    ...
    public void endRound(boolean guessed) {
        ...
        scheduler.schedule(() -> {
            if (gameActive && !users.isEmpty()) startNewRound();
        }, 5, TimeUnit.SECONDS);
    }
    ...
    private void endSession() {
        gameActive = false;
        if (currentTask != null) currentTask.cancel(false);
        if (sessionTask != null) sessionTask.cancel(false);
    ...
    public void handleGuess(String socketId, String text) {
        ...
        if (gameActive && currentWord != null && text.trim().equalsIgnoreCase(currentWord)) {
            ...
            String drawerName = users.get(currentDrawerId);
            if (drawerName != null) scores.put(drawerName, scores.getOrDefault(drawerName, 0) + 5);
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Javalin app = Javalin.create(config -> config.router.mount(router -> {})).start(port);  
        ...
        app.ws("/game", ws -> {
            ...
            ws.onClose(ctx -> {
                ...
                if (rId != null && rooms.containsKey(rId)) rooms.get(rId).removeUser(ctx.sessionId());
            });
            ...
            ws.onMessage(ctx -> {
                ...
                if ("stroke".equals(msg.type)) {
                    if (ctx.sessionId().equals(room.currentDrawerId)) {
                        String json = mapper.writeValueAsString(new Message("remoteStroke", rId, "System", msg.payload));
                    ...
                    if (ctx.sessionId().equals(room.currentDrawerId)) room.broadcast("clearBoard", null);
                } 
    ...
    public static void broadcastToRoom(String roomId, Message msg) {
        userRoomMap.forEach((ctx, rId) -> {
            if (rId.equals(roomId) && ctx.session.isOpen()) ctx.send(msg);
        });
    }
    public static void sendToSocket(String socketId, Message msg) {
        userRoomMap.forEach((ctx, rId) -> {
            if (ctx.sessionId().equals(socketId) && ctx.session.isOpen()) ctx.send(msg);
        });
    }
    public static void removeRoom(String roomId) { rooms.remove(roomId); }
}